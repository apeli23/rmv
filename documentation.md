...
To merge two videos in nextjs, we will of course require two sample videos one of which for simplified demonstration will have to contain a major single colorfill, for example, a unique colored back ground like a green screen video. This move allows us to easily replace the green color with frames from the second video. For this article, we wil use the two samples for our [foreground](https://res.cloudinary.com/dogjmmett/video/upload/v1644764254/foreground_vulcfz.mp4) and [background](https://res.cloudinary.com/dogjmmett/video/upload/v1644764315/background_wjdrt7.mp4) respectively. Paste the videos.
...
With our videos merged, we can capture the processed canvas using a media stream to chunks of blob and passit to the blob state hook created ea
