...
Finaly, use an async function `uploadHandler` to encode the blob to a base 64 string using a file reader and send the encoded file to the backend using a POST method. The upload function's response is then assigned to 